...
<p align="center">
  <img src="./assets/capability-stack.svg" alt="From capability artifact to CLG candidate" width="100%">
</p>

能力能够被单独发布，并不意味着它天然适合增长。

当团队为它设计分发渠道、来源归属、后续承接和测量方式时，它才成为一个
**CLG 候选方案**。这个称呼刻意保留了不确定性：它只是候选方案。实验失败以后，不能用
“那就不算 CLG”把失败样本排除掉。
...
我押注能力会成为独立的产品资产。产品团队会逐渐像管理功能、API 和内容一样，管理能力
的边界、版本、质量、成本、风险和分发。

我暂时不押注能力目录会形成可靠市场，也不押注能力使用天然会带来增长。对当前项目来说，
最有价值的工作不是继续扩大概念，而是选一个现有 Skill，把跨运行时质量、采用成本和增长
增量真正测一遍。

相关来源见[证据地图](./research/evidence-map.md)，实验方案见
[CLG 实验议程](./research/falsification-agenda.md)。
